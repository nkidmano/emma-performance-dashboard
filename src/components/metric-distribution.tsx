...
export function MetricDistribution({
  ...
  const getDistributionCategory = (min: number, max: number | undefined) => {
    ...
    if (max && max <= thresholds.good)
      return { name: "Good", color: colors.good };
    if (min >= thresholds.needsImprovement)
      return { name: "Poor", color: colors.poor };
  ...
  const renderPercentilePin = (
    ...
    return (
      <div
        style={{ left: `${(percentile / (max ?? min)) * 100}%` }}
        className="absolute transform w-[40px] -top-[23px] -translate-x-1/2 z-[10]"
      >
        <p
          className={`leading-none text-xs text-center ${percentileCategory.name === "Good" ? "text-green-800" : percentileCategory.name === "Improvement" ? "text-yellow-800" : "text-red-800"}`}
        >
          {formatValue(percentile)}
        </p>
      ...
      <CardContent>
            ...
            return (
              <div key={index} className="flex flex-col items-center">
                ...
                <span className="text-gray-500">
                  {formatPercentage(dist.proportion)}
                </span>
              </div>
            );
          })}
        ...
        <div className="mt-4 pt-4 border-t border-gray-200 text-xs text-gray-500">
          <div className="flex justify-between">
            <span>
              Good: {"<"} {thresholds.good / (metricName === "CLS" ? 1 : 1000)}
              {metricUnit}
            </span>
            <span>
              Improvement: {thresholds.good / (metricName === "CLS" ? 1 : 1000)}
              -{thresholds.needsImprovement / (metricName === "CLS" ? 1 : 1000)}
              {metricUnit}
            </span>
            <span>
              Poor: {">"}{" "}
